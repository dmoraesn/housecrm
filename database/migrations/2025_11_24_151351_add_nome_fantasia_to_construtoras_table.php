...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('construtoras', function (Blueprint $table) {
            // Adicionando status (se não foi adicionado na correção anterior)
            if (!Schema::hasColumn('construtoras', 'status')) {
                $table->boolean('status')->default(true)->after('email');
            }
            
            // Adicionando nome_fantasia (se não foi adicionado na correção anterior)
            if (!Schema::hasColumn('construtoras', 'nome_fantasia')) {
                $table->string('nome_fantasia')->nullable()->after('nome');
            }

            // Adicionando os campos de Endereço/Extras ausentes (a partir do CEP)
            if (!Schema::hasColumn('construtoras', 'cep')) {
                $table->string('cep', 8)->nullable()->after('status');
            }
    ...
    {
        Schema::table('construtoras', function (Blueprint $table) {
            $table->dropColumn(['situacao', 'socios', 'uf', 'cidade', 'bairro', 'complemento', 'numero', 'logradouro', 'cep', 'status', 'nome_fantasia']);
        });
    }
};