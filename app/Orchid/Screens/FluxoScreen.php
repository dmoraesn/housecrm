...
class FluxoScreen extends Screen
{
    ...
    public function query(Fluxo $fluxo): array
    {
        if ($fluxo->exists) {
            $fluxo = $fluxo->fresh();
            $fluxo->loadMissing('lead', 'construtora');
        } else {
            $fluxo->fill([
                'valor_imovel' => 0,
                'valor_avaliacao' => 0,
    ...
    public function layout(): array
    {
        return [
            ...
            ->title('2. Resumo do Imóvel e Financiamento')
            ->description('Dados financeiros principais para cálculo.')
            ->vertical(),
            ...
            ->title('4. Valores Altos e Balões')
            ->vertical(),
        ...
        if ($status === Fluxo::STATUS_COMPLETED) {
            Proposta::updateOrCreate(
                ...
                [
                    ...
                    'valor_assinatura_contrato' => $fluxo->valor_assinatura_contrato,
