...
<head>
    <meta charset="UTF-8">
    <title>Proposta - {{ $proposta->lead?->nome ?? 'Sem Lead' }}</title>
    <style>
...
<body>
    ...
    <p><strong>Cliente:</strong> {{ $proposta->lead?->nome ?? '—' }}</p>
    <p><strong>Data da Proposta:</strong> {{ $proposta->created_at->format('d/m/Y H:i') }}</p>
