...
Cards de métricas do Dashboard (final)
                ...
                <div class="card-body text-center p-3">
                    ...
                    <h3 class="mb-0 {{ $card['color'] }} fw-bold">
                        @if(isset($card['format']) && $card['format'] === 'currency')
                            {{-- Garante que o valor seja float para a formatação de moeda --}}
                            R$ {{ number_format((float)($card['value'] ?? 0), 2, ',', '.') }}
                        @else
                            {{-- Garante que o valor seja float para formatação de número inteiro --}}
