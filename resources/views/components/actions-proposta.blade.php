<div class="d-flex justify-content-center gap-2">
    ...
    {{-- Gerar PDF --}}
    <a href="{{ $pdfRoute }}" class="text-success" title="Gerar PDF" target="_blank">
        <x-orchid-icon path="bs.file-pdf" class="icon-md" />
    </a>

    {{-- Arquivar (se não estiver arquivado) --}}
    @if (!$isArchived)
        <form method="POST" action="{{ route('platform.propostas.archive') }}" class="d-inline">
            @csrf
            <input type="hidden" name="proposta" value="{{ $archiveId }}">
            <button type="submit" class="btn-icon text-warning" title="Arquivar">
